...
function findFirstString(value: unknown, depth = 0): string | undefined {
...
export function extractDocId(doc: DocRecord) {
...
export function extractDocName(doc: DocRecord) {
...
export function extractPageId(page: PageRecord) {
...
export function extractPageTitle(page: PageRecord) {
...
export function extractPageContent(page: PageRecord) {
  ...
  for (const candidate of directCandidates) {
    ...
    if (candidate && typeof candidate === "object") {
      if (typeof (candidate as Record<string, unknown>).markdown === "string") {
        const markdown = (candidate as Record<string, unknown>).markdown as string
        const trimmed = markdown.trim()
        if (trimmed !== "") {
          return trimmed
        }
      }
      if (typeof (candidate as Record<string, unknown>).text === "string") {
        const text = (candidate as Record<string, unknown>).text as string
...
export function extractHierarchy(doc: DocRecord): DocHierarchy {
  const workspace = (doc.team as Record<string, unknown> | undefined) ?? (doc.workspace as Record<string, unknown> | undefined)
  const space = doc.space as Record<string, unknown> | undefined
  const folder = doc.folder as Record<string, unknown> | undefined
  ...
  const pathSegments = [workspaceName, spaceName, folderName, docName].filter((segment) => Boolean(segment)) as string[]
...
export function inferPageCount(doc: DocRecord, pages: PageRecord[]) {
  ...
  if (Array.isArray(doc.pages)) {
    return (doc.pages as unknown[]).length
  }
  return pages.length
}

export function resolvePreviewLimit(config: ApplicationConfig, override?: number) {
...
export function buildContentPreview(content: string, limit: number) {
...
export function buildDocumentSummary(doc: DocRecord, pageCount: number, pagePreviews: PagePreview[]): DocumentSummary {
