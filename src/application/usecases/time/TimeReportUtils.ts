...
export type PrimaryContextType = "workspace" | "space" | "folder" | "list" | "task"

export type TimeReportContext = {
  ...
  providedContexts?: Record<string, string | undefined>
  tag?: string
  guidance?: string
}
...
export type TimeReport = {
  ...
  perTask: TaskTimeBreakdown[]
  partial: boolean
  partialReasons?: string[]
  missingTaskContextEntries?: number
...
export type TimeReportError = {
  error: {
    type: "invalid_context" | "no_results" | "upstream_error"
    message: string
    details?: unknown
  }
}

type TimeReportBuildOptions = {
...
function normaliseTask(entry: unknown) {
  if (!entry || typeof entry !== "object") {
    return undefined
  }
  const record = entry as Record<string, unknown>
  ...
  const list = taskRecord.list && typeof taskRecord.list === "object" ? (taskRecord.list as Record<string, unknown>) : undefined
  const space = taskRecord.space && typeof taskRecord.space === "object" ? (taskRecord.space as Record<string, unknown>) : undefined
  const folder = taskRecord.folder && typeof taskRecord.folder === "object" ? (taskRecord.folder as Record<string, unknown>) : undefined
  const tags: string[] | undefined = Array.isArray((taskRecord as any).tags)
    ? (taskRecord as any).tags
        .map((tag: unknown) => {
          if (!tag || typeof tag !== "object") return undefined
          const tagRecord = tag as Record<string, unknown>
          if (typeof tagRecord.name === "string") return tagRecord.name
          if (typeof tagRecord.tag === "string") return tagRecord.tag
...
function matchesPrimaryContext(
  task: ReturnType<typeof normaliseTask>,
  context: TimeReportContext
): boolean {
  ...
  if (context.contextType === "task") {
    if (task.taskId === context.contextId) return true
    if (context.includeSubtasks && task.parentId === context.contextId) return true
    return false
  }
  return false
}
...
export async function buildTimeReport(options: TimeReportBuildOptions): Promise<TimeReport | TimeReportError> {
  ...
  const perTask = new Map<string, TaskTimeBreakdown>()
